...
io.sockets.on('connection', function (socket) {
	...
	function generate(data) {
		...
		var map = function (d) {
			...
			for (p = data.schritt_min; p < data.schritt_max; p += 1) {
				...
				if (
					d.value[c] && d.value[c][cdrill]
				&&	d.value[n] && d.value[n][ndrill]
				) {
					...
					nodes[p + ' - ' + d.value[c][cdrill]].count 		+= (data.sum === '*' ? d.value.count : d.value[data.sum]) || 1;
					nodes[(p + 1) + ' - ' + d.value[n][ndrill]].count 	+= (data.sum === '*' ? d.value.count : d.value[data.sum]) || 1;
	...
	socket.on(
		'help_content',
		function (data) {
			mongo.collection(data.content).find({metadata: {$exists: true}}).toArray(
				function (err, list) {
					if (list.length > 0) {
						data.base = list;
						socket.emit(
	...
	socket.on(
		...
		function (data) {
			...
			important = {};
			hidden = {};	
			mongo.collection(data.base).find({metadata: {$exists: true}}).toArray(
				function (err, list) {
					if (list.length > 0) {
						result = [{
						...
						mongo.collection(data.base).aggregate([
							{
								$group: {
									_id: '',
									max_date: { $max: '$date' },
